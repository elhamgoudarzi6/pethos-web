...
export class PropertyAddComponent implements OnInit {
  ...
  getUsers(): any {
    this.service
      .getAllUsers(this.localStorage.userToken)
      .subscribe((response) => {
        if (response.success === true) {
  ...
  createForm() {
    this.form = new FormGroup({
      ...
      priceRent: new FormControl(null),
      priceMortgage: new FormControl(null),
  ...
  submitForm(): void {
    ...
    this.service
      ...
      .subscribe((discountRes) => {
        if (discountRes.success === true) {
          this.form.controls.discountID.setValue(discountRes.data[0]._id);

          // fill agentId randomly
          this.service
            .getAllAgentsByCategoryId(
              this.localStorage.userToken,
              this.form.controls.subPropertyTypeID.value
            )
            .subscribe((agentRes) => {
              if (agentRes.success === true) {
                if (agentRes.data.length === 0) {
                  this.messageService.add({
                    ...
                    detail: agentRes.data,
                  });
                  return;
                } else if (agentRes.data.length === 1) {
                  this.form.controls.agentID.setValue(agentRes.data[0]._id);
                } else {
                  this.form.controls.agentID.setValue(
                    agentRes.data[this.generateRandom(agentRes.data.length)]._id
                  );
                }
                ...
                // add to db
                this.service
                  .addProperty(this.localStorage.userToken, this.form.value)
                  .subscribe((response) => {
                    if (response.success === true) {
                      this.ref.close(true);
                    } else {
  ...
  stateOnChange(event: any): void {
    this.form.controls.state.setValue(event.value.name);
    const code = event.value.code;
    this.fillCities(code);
  }

  cityOnChange(event: any): void {
    this.form.controls.city.setValue(event.value.name);
  }

  setStateCityDefaultValue() {
    this.form.controls.state.setValue({ code: '21', name: 'لرستان' });

    this.fillCities('21');
    this.form.controls.city.setValue({ code: '297', name: 'خرم آباد' });
  }

  priceTypeOnChange(event: any): void {
    this.form.controls.priceType.setValue(event.value.name);
  }
}
